# 하이프랜드 키바인드 — 무엇을 부를 수 있고, 무엇이 조용히 실패하는가
...
죽은 바인드 셋이 한동안 살아남았다. 이 문서는 그 빈자리를 채운다.

**어느 키가 무엇을 하는지는 여기 없다.** 그건 세 창 관리자가 공유하므로 정본이
하나뿐이다 — [`keymap.md`](keymap.md). 이 문서는 그 아래층, 하이프랜드에서만 참인
것만 다룬다.
...
두 자리는 **의도적으로 비어 있다.** 채우기 전에 이유를 볼 것:

- `Mod+Shift+E` — 로그아웃이 걸려 있던 곳. `Mod+E`(오버뷰)와 `Mod+Shift+F` 사이라
  오타 한 번이면 세션이 통째로 날아간다. 아래 "뺀 것" 참고.
- `Mod+Ctrl` 의 글자 대부분 — 이 티어는 "모니터를 넘는다"는 뜻을 갖고 있다
  (`keymap.md`). 아무 기능이나 넣으면 티어의 의미가 흐려진다.

새 키를 고를 때는 **세 창 관리자에서 같이 비어 있는지** 봐야 한다. 한쪽에만 넣으면
그게 다음번 근육기억 충돌이 된다.
...
이것이 DMS 자체 키바인드 묶음(`dms/binds.lua`)을 **일부러 안 읽는** 이유다. 그대로
두면 겹치는 키가 토글을 두 번 때려 아무 일도 안 일어난다. 필요한 IPC 호출은
`hyprland.lua` 에 직접 적었고, 셸이 만드는 치트시트(`Mod+H`)는 그 파일을 읽으므로
목록은 그대로 나온다.
