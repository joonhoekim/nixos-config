...
-- 세 손가락 가로 스와이프로 워크스페이스 전환. **트랙패드 전용이다** —
-- 손가락으로 화면을 쓸어도 이 줄은 안 탄다. 0.56 의 제스처 엔진은 통째로
-- 트랙패드 것이고(바이너리 심볼이 전부 `C*TrackpadGesture` 다), 터치스크린은
-- 아래 workspace_swipe_touch 라는 구형 경로로만 워크스페이스를 넘긴다.
hl.gesture({ fingers = 3, direction = "horizontal", action = "workspace" })

-- 터치스크린. 터치 장치가 없는 머신에서는 통째로 무의미하고 에러도 안 난다.
hl.config({
    input = {
        touchdevice = {
            -- 디지타이저를 내장 패널에 못 박는다. 이 줄이 없으면 외장 모니터를
            -- 붙이는 순간 터치 좌표가 **전체 레이아웃**(내장+외장을 합친 넓이)에
            -- 매핑되어, 화면을 찍으면 가로로 눌린 엉뚱한 자리가 눌린다. 모니터
    ...
    gestures = {
        -- 손가락 가로 스와이프로 워크스페이스. 위 hl.gesture 와 별개의 경로이고,
        -- 터치스크린에서 워크스페이스를 넘기는 방법은 0.56 기준 이것뿐이다.
